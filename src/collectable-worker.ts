...
const finalizers = new FinalizationRegistry((x: any) => x._clean());

/**
 * A Worker that will be terminated by the garbage collector.
 */
export class CollectableWorker {
    /**
     * Construct a collectable worker.
     * @param target  Object to finalize on.
     * @param url  URL of Worker to load.
     */
    constructor(target: object, url: string) {
        const worker = this._worker = new Worker(url);
        ...
        finalizers.register(target, this);
    }
